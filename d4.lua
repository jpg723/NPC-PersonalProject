...
function scene:create( event )
	...
	local function timeAttack(event) 
		...
		if limit <= 0 then
			composer.setVariable("correct4", score)
			composer.gotoScene("m5")
		end
	end
    
	tm = timer.performWithDelay(1000, timeAttack, limit)

	local num7 = math.random(100)
	local num8 = math.random(50)

	for i = 1, 2 do
		if (num7 == num1 and num8 == num2) or (num7 == num2 and num8 == num1) 
		or (num7 == num3 and num8 == num4) or (num7 == num4 and num8 == num3) 
		or (num7 == num5 and num8 == num6) or (num7 == num6 and num8 == num5) then
			num7 = math.random(100)
			num8 = math.random(50)
		end
	end
	local numX = display.newText(num7 , display.contentCenterX - 200, display.contentHeight*0.37, "font/123RF_하르방_R_TTF.ttf")
	numX:setFillColor(0)
	numX.size = 70
	...
	local numY = display.newText(num8 , display.contentCenterX, display.contentHeight*0.37, "font/123RF_하르방_R_TTF.ttf")
	numY:setFillColor(0)
	numY.size = 70
	...
	rec1:setStrokeColor(0.5)
   	...
   	local answer1 = display.newText(string.format("%6.1f",nums[1]) , display.contentCenterX - 375, display.contentCenterY + 145, "font/123RF_하르방_R_TTF.ttf")
	...
	rec2:setStrokeColor(0.5)
   	...
   	local answer2 = display.newText(string.format("%6.1f",nums[2]) , display.contentCenterX - 15, display.contentCenterY + 145, "font/123RF_하르방_R_TTF.ttf")
	...
	rec3:setStrokeColor(0.5)
   	...
   	local answer3 = display.newText(string.format("%6.1f",nums[3]) , display.contentCenterX + 330, display.contentCenterY + 145, "font/123RF_하르방_R_TTF.ttf")
	answer3:setFillColor(0)
	answer3.size = 60 
	sceneGroup:insert(answer3)

	local result = num7 / num8
	local function selectAnswer4(event)
		if nums[1] == result then
			score = 1
		end
		composer.setVariable("correct4", score)
		composer.gotoScene("d5")
	end
	rec1:addEventListener("tap", selectAnswer4)

	local function selectAnswer5(event)
	...
	rec2:addEventListener("tap", selectAnswer5)

	local function selectAnswer6(event)
	...
	rec3:addEventListener("tap", selectAnswer6)

	composer.setVariable("num7", num7)
	composer.setVariable("num8", num8)
end
